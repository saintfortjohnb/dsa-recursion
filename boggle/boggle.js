...
function find(board, word) {
  ...
  function dfs(r, c, k) {
      ...
      if (k === word.length - 1) return true;  // word is found

      const temp = board[r][c];
      board[r][c] = '/';  // mark as visited

      const directions = [[-1, 0], [1, 0], [0, -1], [0, 1]]; // NEWS directions
      for (let dir of directions) {
          if (dfs(r + dir[0], c + dir[1], k + 1)) {
              return true;
          }
      }

      board[r][c] = temp;  // unmark visited, backtrack
      return false;
  }
